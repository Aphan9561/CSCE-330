...
%% grand_parent(X, Z): X is a grand parent of Z.
...
full_sibling(X, Y) :- mother(M, X), father(F,X), mother(M, Y), father(F,Y), M==M, F==F, X\==Y.

%% Half siblings are siblings who have exactly one parent in common. 
%% Problem 5. Define the half_sibling predicate. (4 points)
%% Hints: use mother and father predicates.
half_sibling(X, Y) :- (mother(M, X); father(F, X)), (mother(M, Y); father(F, Y)), X\==Y.
...
solve_digits(C, R, O, S, A, D, N, G, E) :- 
    ...
    C1 is (S+S) // 10,
    dig(D), \+ R = D, \+ S = D, 
    E is (S+D+C1) mod 10, \+ S = E, \+ D = E, \+ R = E, 
    C10 is (S+D+C1) // 10,
    dig(O), dig(A), \+ O = A, \+ O = S, \+ O = D, \+ O = E, \+ S = A, \+ R = A, \+ R = O, \+ A = E, \+ A = D,
    G is (O+A+C10) mod 10, \+ D = G, \+ R = G, \+ A = G, \+ S = G, \+ G = E, \+ O = G,
    C100 is (O+A+C10) // 10,
    N is (R+O+C100) mod 10, \+ N = G, \+ N = E, \+ D = N, \+ A = N, \+ S = N, \+ O = N, \+ R = N,
    C1000 is (R+O+C100) // 10,
    dig(C), \+ C = R, \+ C = O, \+ C = S, \+ C = A, \+ C = D, \+ C = N, \+ C = G, \+ C = E,
    A is (R+C+C1000) mod 10, D is (R+C+C1000) // 10.
...
solve(Coffee, Water, Coke, Milk, Steak, Lasagna, Pizza, Chicken) :- 
    people(Coffee), people(Water), people(Coke), people(Milk), 
    people(Steak), people(Lasagna), people(Pizza), people(Chicken),
    \+ Coffee = Milk,
    \+ Coffee = Water,
    \+ Coffee = Coke,
    \+ Water = Milk,
