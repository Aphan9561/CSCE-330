...
%% distinct/1, print/1, write/1, format/2, nl/0 and fail/0 in your queries.
...
%% 2 points for acted_in(bill_murray, lost_in_translation), if you write acted_in(bill_murray, X), this is also fine. 
...
%% 2 points for directed(blade_runner, X).  
query2 :- directed(blade_runner, X), print(X), nl, fail.
...
%% 1 point each for acted_in(X, home_alone_3), acted_in(X, north). 
query3 :- acted_in(X, home_alone_3), acted_in(X, north), print(X), nl, fail. 
...
%% 1 point each for released(X, 1990), \+ acted_in(al_pacino, X). 
query4 :- released(X, 1990), \+ acted_in(al_pacino, X), print(X), nl, fail.
...
%% 1 point each for released(Z, 2004), directed(Z, Y). 
query5 :- released(Z, 2004), directed(Z, Y), format("~s directed ~s\n", [Y, Z]), fail. 

%% Question 6. Has anyone directed more than 2 movies? (3 points)

%% 1 point for directed(X1, Y), directed(X2, Y), directed(X3, Y).
%% 1 point for the inequalities. 
%% 1 point for trying. 

%% Auxillary predicates (optional): 
multi_movies(Y) :-
    directed(X1, Y), directed(X2, Y), directed(X3, Y),
    \+ X1 = X2, \+ X1 = X3, \+ X2 = X3.

%% Queries for Question 6, please replace "undefined" with your query.

query6 :- distinct(multi_movies(Y)), print(Y), nl, fail.

%% Question 7. Does any movie have more than one director? (3 points)

%% 1 point for directed(Movie, Director1), directed(Movie, Director2). 
%% 1 point for the inequality.
%% 1 point for trying.
%% Auxillary predicates (optional): 
multi(Movie) :-
    directed(Movie, Director1), directed(Movie, Director2),
    \+ Director1 = Director2.

%% Queries for Question 7, please replace "undefined" with your query.

query7 :- distinct(multi(Y)), print(Y), nl, fail.

%% Question 8. Has anyone acted in more than one movie released in 2006? (3 points)

%% 1 point for released(X1, 2006), released(X2, 2006). 
%% 1 point for acted_in(Y, X1), acted_in(Y, X2). 
%% 1 point for the inequality.

%% Auxillary predicates (optional): 
multi_act(Y) :-
    released(X1, 2006), released(X2, 2006),
    acted_in(Y, X1), acted_in(Y, X2), \+ X1 = X2.

%% Queries for Question 8, please replace "undefined" with your query. 
query8 :- distinct(multi_act(Y)), print(Y), nl, fail. 

%% Question 9. Has anyone acted in more than two movies in the same year? (3 points)

%% Auxillary predicates (optional):
%% 1 point for released(X1, Y), released(X2, Y), released(X3, Y).
%% 1 point for acted_in(Z, X1),acted_in(Z, X2), acted_in(Z, X3). 
%% 1 point for the inequalities.

multi_act_same(Z, Y) :-
    released(X1, Y), released(X2, Y), released(X3, Y),
    \+ X1 = X2, \+ X1 = X3, \+ X2 = X3,
    acted_in(Z, X1),acted_in(Z, X2), acted_in(Z, X3).

%% Queries for Question 9, please replace "undefined" with your query.
query9 :- distinct(multi_act_same(Actor, Year)), format("~s in ~d\n", [Actor, Year]), fail.

%% Question 10. Who has worked with the same director in different years? (3 points)

%% 1 point for directed(X1, Director) and directed(X2, Director).
%% 1 point for released(X1, Year1), released(X2, Year2).
%% 1 point for the inequality and acted_in(Y, X1), acted_in(Y, X2). 

%% Auxillary predicates (optional): 
collab(Y, Director, Year1, Year2) :-
    directed(X1, Director),
    directed(X2, Director),
    released(X1, Year1),
    released(X2, Year2),
    \+ Year1 = Year2,
    acted_in(Y, X1), acted_in(Y, X2). 

%% Queries for Question 10, please replace "undefined" with your query. 
query10 :-
    distinct(collab(Y, Director, Year1, Year2)),
    format("~s with ~s in ~d and ~d\n", [Y, Director, Year1, Year2]), fail. 
